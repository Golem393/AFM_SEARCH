...
print("Load Gemma model:...", end=' \r')
pipe = pipeline("text-generation", 
                model="google/gemma-3-1b-it", 
                device=args.device, 
                torch_dtype=torch.bfloat16)
print("Load Gemma model: success", end=' \r')

# System propmpt for Gemma model. Can be optimized in future (max. 32K Token context)
...
print(f"Extracted keywords: {kw_string}")
...
print("Save keyword embeddings:...", end='\r')
np.save("query_embeddings.npy", query_emb)
print("Save keyword embeddings: success")